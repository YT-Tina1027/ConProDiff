...
def convert_to_seq(x, alphabet):
    ...
    if x.ndim != 2:
        raise ValueError(f"convert_to_seq expects 2D array after squeeze, got shape {x.shape}")
    ...
    if x.shape[0] == 4:
        ...
        raise ValueError(f"convert_to_seq expects one dimension to be 4, got shape {x.shape}")
