...
COND_PALETTE = {
    ...
    "high": {
        "fill_light": "#F5DDE0",   # 浅玫粉
        "fill_mid":   "#E8AABA",   # 中玫粉
        "fill_dark":  "#D4758A",   # 深玫粉
        "line":       "#B84D65",
        "marker":     "#8A2A42",
    },
}
...
def plot_fig_h(top_n=12):
    ...
    # 与 E.coli 一致：玫粉系渐变
    light = COND_PALETTE["low"]["fill_light"]   # "#F5DDE0"
    dark  = COND_PALETTE["low"]["fill_dark"]    # "#D4758A"
    ...
    with plt.rc_context(NATURE_RC):
        ...
        im = ax.imshow(df_top.values, aspect="auto", cmap=cmap,
                       vmin=0, vmax=0.3,               # ← 固定为 1（与 E.coli 一致）
        ...
        ax.set_ylabel("TF Motif", fontsize=9)                          # ← 9
        ...
        cbar.set_label("Fraction of sequences with motif", fontsize=8) # ← 8
...
# =============================================================
# Fig I — Motif 多样性：生成 vs 真实（violin + boxplot）
# =============================================================
def plot_fig_i():
        ...
        for shift, source, n_motifs_dict, alpha_v, alpha_b in [
            ...
            for i, (lbl, patch) in enumerate(zip(LABEL_ORDER, bp["boxes"])):
                ...
                ax.text(positions[i], med + 0.15, f"{med:.0f}",
                        ha="center", va="bottom",
                        fontsize=9, color="white")   # ← 9（与 E.coli 一致）
...
def plot_fig_j():
    ...
    with plt.rc_context(NATURE_RC):
        ...
        ax.set_title("Differential TFBS Enrichment:\nLow vs High Promoter Strength",
                     fontsize=11)
        ax.legend(fontsize=9, loc="upper right",          # ← lower right（与 E.coli 一致）
