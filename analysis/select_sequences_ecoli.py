...
核心逻辑（去除所有生物学硬过滤，只保留 pred 分层 + 多样性）：
  ...
  3. 天然对照（nature）：
       high → pred 不超过生成 high 组最低值
       mid  → pred 落在 [q33, q67] 内
       low  → pred 不超过生成 low 组最高值
  4. 随机阴性对照：pred 严格低于生成 low 组最低值（尽量）
  5. 最终保证：random ≤ generated_low ≤ generated_mid ≤ generated_high
                nature_xxx ≤ generated_xxx（各组独立上限）
"""

import os, sys, gc as _gc, re, warnings, logging, pickle
import numpy as np
import pandas as pd
...
# 天然对照文件路径（可选，有则读入并预测；空则跳过）
NATURE_CSV = ""  # 若真实 CSV 即为天然对照来源，留空后由 REAL_CSV 提供

OUT_DIR = "/home/yt/Code/DNA-Diffusion/result/experiments_ecoli/selected_sequences"
...
N_RANDOM       = 2      # 随机阴性对照数量
N_RANDOM_POOL  = 500    # 随机候选池大小（越大越易找到低活性序列）
...
def predict_sequences(sequences):
    ...
    arrs = [torch.tensor(encode(s)) for s in sequences]
    x = torch.stack(arrs).to(device)
    with torch.no_grad():
        y = scaler.inverse_transform(
            model(x).detach().cpu().numpy().reshape(-1, 1)
        ).reshape(-1)
...
def compute_bin_edges(real_csv):
    ...
    col = next((c for c in ["strength", "activity", "score", "expression"]
                if c in df.columns), None)
    if col is None:
        raise ValueError(f"找不到表达强度列，当前列：{list(df.columns)}")
    vals = pd.to_numeric(df[col], errors="coerce").dropna().values
    q33, q67 = np.percentile(vals, 33.333), np.percentile(vals, 66.667)
...
def select_generated(seqs, preds, group, q33, q67, n=5, pool=80):
    ...
    if "dist" in result.columns:
        result = result.drop(columns=["dist"])
...
# =========================================================
# 天然对照筛选
# 约束：pred 不超过对应生成组的 pred 范围（不优于生成序列）
# =========================================================
...
def select_nature(real_csv, group, q33, q67,
    ...
    """
    从 REAL_CSV 中选天然对照。
    上限：pred ≤ gen_pred_max（不优于生成序列最高）
    下限：pred ≥ gen_pred_min（不低于生成序列最低，保证在同档位）
    实际上用 [gen_pred_min, gen_pred_max] 范围内取最接近中心的序列。
    """
    df = pd.read_csv(real_csv)
    df.columns = [c.strip().lower() for c in df.columns]
    ...
    if len(qualified) < n:
        # 放宽：只保证不超过 gen_pred_max（上限不变，下限去掉）
        qualified = df[df["pred"] <= gen_pred_max].copy()
        print(f"  [nature/{group}] ⚠ 严格范围内不足 {n} 条，放宽下限（上限保持 ≤{gen_pred_max:.4f}）")
...
# =========================================================
# 随机阴性对照
# 约束：pred 严格低于生成 low 组最低值
# =========================================================

def select_random(low_pred_min, n=2, pool_size=500, length=165, seed=42):
    ...
    for multiplier in [1, 2, 4]:
        ...
        print(f"  [random] 生成 {cur_pool} 条随机序列，预测中…")
        preds = predict_sequences(seqs)
        ...
        if len(strict) >= n:
            ...
            print(f"  [random] 仅 {len(strict)} 条满足 pred < {low_pred_min:.4f}，"
                  f"扩大候选池重试…")
            if multiplier == 4:
                # 最后保底：取 pred 最低的 n*5 条
                df = df.head(max(n * 5, 10))
                print(f"  [random] ⚠ 保底：取 pred 最低的 {len(df)} 条，"
                      f"最低={df['pred'].min():.4f}（仍高于 {low_pred_min:.4f}，"
                      f"建议增大 N_RANDOM_POOL）")
...
def check_pred_hierarchy(frames):
    """
    打印各组 pred 范围，检查 random ≤ low ≤ mid ≤ high 是否成立。
    """
    print("\n  [pred 层次检查]")
    order = ["random", "low", "mid", "high"]
    stats = {}
    for grp in order:
        sub = pd.concat(frames)[pd.concat(frames)["group"] == grp]
        if not sub.empty:
...
def main():
    ...
    all_preds = predict_sequences(all_seqs)
    ...
    # 5. 天然对照（pred 上限 = 对应生成组的 pred 最高值）
    nat_parts = []
    if N_NATURE > 0:
    ...
    # 8. 保存
    csv_path = os.path.join(OUT_DIR, "selected_EC_simple.csv")
    fa_path  = os.path.join(OUT_DIR, "selected_EC_simple.fasta")
