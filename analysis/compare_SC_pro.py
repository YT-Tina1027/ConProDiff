...
OUT_DIR      = "/home/yt/Code/DNA-Diffusion/compare_SC_pub"
...
def compute_thresholds(path):
    ...
    if col is None:
        raise ValueError(f"No strength column in {path}, cols={df.columns.tolist()}")
...
def load_generated(q1, q2):
    ...
    for method, label, path in [
        ...
        for s in read_txt(path):
            rows.append({"method":method,"label":label,"sequence":s,"strength":np.nan,"oracle_score":np.nan})
...
def load_oracle():
    ...
    g = tf.Graph()
    with g.as_default():
        model, scaler, batch_size = load_model(ORACLE_COND)
    return g, model, scaler, batch_size
...
def attach_oracle(df):
    print("Loading oracle …")
    g, model, scaler, bs = load_oracle()
    parts = []
    for (method, label), sub in df.groupby(["method","label"]):
        ...
        sub["oracle_score"] = oracle_score(seqs, g, model, scaler, bs)
        parts.append(sub)
    return pd.concat(parts, ignore_index=True)
    ...
    for method in GEN_METHODS:
        for label in LABEL_ORDER:
            ...
            rows.append({
                ...
                "GC mean":        round(float(np.mean([gc(s) for s in sub.sequence])),3),
...
# ══════════════════════════════════════════════════════════════
# 7.  Figure 1 – Oracle score violin + jitter
# ══════════════════════════════════════════════════════════════
...
def fig_violin(df, TR):
    ...
    axes[0].set_ylabel("Oracle expression score")
    ...
    fig.suptitle("Oracle expression score distribution by target condition",
                 y=1.12, fontsize=10, fontweight="bold")
    fig.tight_layout()
    savefig(fig, "Fig1_oracle_violin")

# ══════════════════════════════════════════════════════════════
# 8.  Figure 2 – Success rate + Intra-group edit distance
# ══════════════════════════════════════════════════════════════
...
def fig_success_and_diversity(df, TR):
    ...
    ax1.set_title("Conditional controllability", fontweight="bold")
    ...
    ax2.set_title("Sequence diversity (edit distance)", fontweight="bold")

    handles2, labels2 = ax2.get_legend_handles_labels()
    handles2.append(plt.Line2D([0], [0], color="#3C5488", ls="--", lw=1.6))
    labels2.append("Real (baseline)")
    ax2.legend(handles2, labels2, loc="upper right", fontsize=7.5)

    fig.tight_layout()
    savefig(fig, "Fig2_controllability_diversity")

# ══════════════════════════════════════════════════════════════
# 9.  Figure 3 – GC content + 4-mer fidelity panel
# ══════════════════════════════════════════════════════════════
...
def fig_gc_kmer(df):
    ...
    for label in LABEL_ORDER:
        for method in METHOD_ORDER:
            vals = df2[(df2.method==method)&(df2.label==label)]["gc"].dropna().values
    ...
    ax2.set_title("4-mer distributional fidelity", fontweight="bold")
...
# ══════════════════════════════════════════════════════════════
# 10.  Figure 4 – Multi-metric heatmap
...
def fig_heatmap(df, TR):
    ...
    def draw_method_heatmap(ax, mat, norm_mat, method, show_yticks):
        ...
        ax.set_title(METHOD_LABEL[method], fontweight="bold",
                     fontsize=10, pad=6, color=base_col)
    ...
    fig.suptitle("Multi-metric comparison: Our model vs. PromoDGDE",
...
def make_tables(df, TR):
    ...
    to_latex(metrics,
             ...
             caption="Comparison of oracle expression scores, controllability, and GC content "
                     "between our end-to-end model and PromoDGDE. "
                     "MWU: Mann–Whitney U test versus Real sequences; *, **, *** denote "
    ...
    to_latex(kmer_wide,
             ...
             caption="k-mer distributional fidelity (Pearson r between generated and real "
                     "k-mer frequency vectors) for k=3,4,5.",
             label="tab:kmer")
...
def main():
    print("="*60)
    print("Publication-quality comparison: our vs PromoDGDE")
    print("="*60)
    ...
    print("\n[2/4] Oracle scoring …")
    all_df = attach_oracle(all_df)
    all_df["gc"] = all_df["sequence"].apply(gc)
